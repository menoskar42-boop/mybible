                ...
                <div className="space-y-2">
                    ...
                    <motion.div key={`${v.id}-${i}`} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}>
                      <div className="p-3 border border-border rounded-lg hover:border-primary/40 transition-colors">
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <Badge variant="secondary" className="text-xs">{v.bookName} <span dir="ltr">{v.chapter}:{v.verse}</span></Badge>
                  ...
                  {searchMore.map((v, i) => (
                    <motion.div key={`more-${i}`} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
                      <div className="p-3 border border-border rounded-lg hover:border-primary/40 transition-colors">
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <Badge variant="secondary" className="text-xs">{v.book} <span dir="ltr">{v.chapter}:{v.verse}</span></Badge>
                ...
                {draft.map((v, i) => (
                  <div key={v.id} className="p-2 bg-muted/40 rounded-lg text-xs">
                    <div className="flex items-start justify-between gap-2">
                      <span className="font-semibold text-primary shrink-0">{i + 1}. {v.bookName} <span dir="ltr">{v.chapter}:{v.verse}</span></span>
